...
DATASET_FILE = Path(
    "data"
) / "processed" / "global_mgsm_ar_en_100.jsonl"

MODEL_CONFIGS = {
    "qwen": {
        "model_name": "Qwen/Qwen2.5-Math-1.5B-Instruct",
        "output_file": "qwen_mgsm_100_results.csv",
    },

    "gemma": {
        "model_name": "google/gemma-3-1b-it",
        "output_file": "gemma_mgsm_100_results.csv",
    },
}
...
if len(records) != 100:
    raise RuntimeError(
        f"Expected 100 problems, found {len(records)}."
    )


print("=" * 76)
print("MGSM 100-PROBLEM BILINGUAL REASONING EXPERIMENT")
print("=" * 76)
...
def extract_answer(response):
    ...
    We choose the FIRST explicit final-answer marker.

    Earlier experiments showed that both Qwen and Gemma can
    correctly answer and then enter repetitive generation.
    Choosing the final repeated marker produced false negatives.

    If multiple explicit markers disagree, the row is flagged
    for manual review.
    """
...
print("\n" + "=" * 76)
print("STARTING / RESUMING EVALUATION")
print("=" * 76)
...
for record in records:
    ...
    # Alternate order to avoid one language
    # systematically being generated first.
...
for row in review_cases:
    ...
    print(
        f"Problem {int(row['problem_id']):3d} | "
        f"{row['language']:7s} | "
...
print(
    ...
    "manual-review cases and suspicious failures have "
    "been checked."
)